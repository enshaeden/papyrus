...
from papyrus.interfaces.web.view_helpers import escape, join_html, link, quoted_path, slugify


INGEST_STAGES = ("uploaded", "parsed", "classified", "mapped", "reviewed")


def _stage_progress_html(runtime, *, current_stage: str) -> str:
    items = []
    ...
    return runtime.template_renderer.render(
        "partials/progress_bar.html",
        {
            ...
            "items_json": escape(__import__("json").dumps(items, ensure_ascii=True)),
        },
    )


def _ingest_list_page(runtime, *, request: Request) -> str:
    ...
    rows = "".join(
        (
            ...
            f"<td>{escape(item['updated_at'])}</td></tr>"
        )
        for item in ingestions
    )
    table_html = (
        f'<table class="data-table"><thead><tr><th>File</th><th>Status</th><th>Blueprint</th><th>Updated</th></tr></thead><tbody>{rows}</tbody></table>'
        if ingestions
...
def _ingestion_detail_page(runtime, *, request: Request, detail: dict[str, object]) -> str:
    ...
    return runtime.page_renderer.render_page(
        ...
        page_context={
            "progress_html": _stage_progress_html(runtime, current_stage=str(detail["status"])),
...
def _mapping_review_page(runtime, *, request: Request, detail: dict[str, object], mapping: dict[str, object], errors: list[str] | None = None) -> str:
    ...
    return runtime.page_renderer.render_page(
        ...
        page_context={
            "progress_html": _stage_progress_html(runtime, current_stage="mapped"),
        ...
        if request.method == "POST":
            ...
            if upload is None and not source_path:
                return html_response(
                    _ingest_list_page(runtime, request=request),
                )
            result = (
            ...
            return redirect_response(
                f"/ingest/{quoted_path(result['ingestion_id'])}?notice={quote_plus('Ingestion completed through parse, classification, and initial mapping.')}"
            )
        return html_response(_ingest_list_page(runtime, request=request))
