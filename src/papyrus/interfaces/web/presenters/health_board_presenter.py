...
def item_href(item: dict[str, Any]) -> str:
...
def render_health_column(title: str, summary: str, items: list[dict[str, Any]], *, tone: str) -> str:
    body = (
        join_html(
            [
                (
                    ...
                    f'<h3>{link(item["title"], item_href(item))}</h3>'
                    ...
                    f'{link("Open", item_href(item), css_class="button button-ghost", attrs={"data-action-id": "open-primary-surface"})}'
                    "</article>"
                )
                for item in items[:6]
            ]
...
def render_health_board(*, queue: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        + render_health_column("Trust debt", "Weak, stale, or suspect guidance that changes whether operators should trust the surface.", groups["trust_debt"], tone="danger")
        + render_health_column("Review backlog", "Guidance waiting on explicit governance decisions.", groups["review_backlog"], tone="warning")
        + render_health_column("Ownership gaps", "Articles that need clearer stewardship before they age into drift.", groups["ownership_gaps"], tone="brand")
        + render_health_column("Stable watch", "Items worth monitoring without immediate intervention.", groups["stable_watch"], tone="default")
        + "</div></section>"
    )
