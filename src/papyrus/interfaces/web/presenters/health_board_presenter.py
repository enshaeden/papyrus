...
def render_health_column(title: str, summary: str, items: list[dict[str, Any]], *, role: str, tone: str) -> str:
    body = (
        join_html(
            [
                (
...
def render_health_board(*, role: str, queue: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        + render_health_column("Ownership gaps", "Articles that need clearer stewardship before they age into drift.", groups["ownership_gaps"], role=role, tone="brand")
        + render_health_column("Stable watch", "Items worth monitoring without immediate intervention.", groups["stable_watch"], role=role, tone="default")
        + "</div></section>"
    )
