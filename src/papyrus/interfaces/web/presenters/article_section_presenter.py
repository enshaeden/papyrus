...
def render_article_section(*, section: dict[str, Any]) -> str:
    ...
    body_html = join_html([render_article_block(block) for block in blocks]) if blocks else f'<p class="article-empty">{escape(section.get("empty") or "No article content recorded.")}</p>'
    return (
