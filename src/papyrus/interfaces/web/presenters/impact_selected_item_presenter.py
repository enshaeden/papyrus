...
from papyrus.interfaces.web.view_helpers import escape, join_html, quoted_path, link


def render_impact_selected_item(*, item: dict[str, Any], surface: str) -> str:
    return (
        ...
        + f'<div class="impact-selected-item__footer">{link("Open guidance", f"/objects/{quoted_path(str(item["object_id"]))}", css_class="button button-secondary")}</div>'
        + "</section>"
    )
