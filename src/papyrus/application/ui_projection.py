...
def workflow_projection_payload(projection: WorkflowProjection) -> dict[str, Any]:
    return {
        ...
        "reasons": list(projection.reasons),
        "actions": [
            action_descriptor_payload(action)
...
def build_use_guidance(
    *,
    ...
    if source_sync_state == SourceSyncState.PENDING.value:
        safe_to_use = (
            revision_review_state == RevisionReviewState.APPROVED.value and trust_state == "trusted"
        )
        return UseGuidanceProjection(
    ...
    if revision_review_state == RevisionReviewState.APPROVED.value and trust_state == "trusted":
        return UseGuidanceProjection(
            ...
            next_action=str(
                posture.get("approval", {}).get("action") or "Use the current guidance."
            ),
            safe_to_use=True,
        )
    if revision_review_state == RevisionReviewState.IN_REVIEW.value:
        return UseGuidanceProjection(
            ...
            next_action=str(
                posture.get("approval", {}).get("action")
                or "Use the last approved guidance or route the revision through review."
            ),
            safe_to_use=False,
        )
    ...
    if trust_state == "weak_evidence":
        return UseGuidanceProjection(
            ...
            next_action=str(
                posture.get("approval", {}).get("action")
                or "Review citations and capture stronger evidence before relying on this guidance."
            ),
            safe_to_use=False,
        )
    if trust_state == "stale":
        return UseGuidanceProjection(
            ...
            next_action=str(
                posture.get("approval", {}).get("action")
                or "Refresh the content and validate it before relying on it."
            ),
            safe_to_use=False,
        )
    if trust_state == "suspect":
        return UseGuidanceProjection(
            ...
            detail=str(
                posture.get("trust_detail")
                or "Papyrus marked this guidance suspect and it needs explicit review."
            ),
            next_action=str(
                posture.get("approval", {}).get("action")
                or "Inspect the audit trail and related dependencies before use."
            ),
            safe_to_use=False,
        )
    return UseGuidanceProjection(
        ...
        detail=str(
            posture.get("trust_detail") or "Papyrus cannot confirm the current governance posture."
        ),
        next_action=str(
            posture.get("approval", {}).get("action")
            or "Inspect the detail and audit views before acting."
        ),
        safe_to_use=False,
    )
...
def build_ingestion_actions(*, detail: dict[str, Any]) -> tuple[ActionDescriptor, ...]:
    ...
    converted = (
        bool(detail.get("converted_revision_id"))
        or ingestion_state == IngestionStatus.CONVERTED.value
    )
    actions: list[ActionDescriptor] = [
        action_from_gate(
...
def build_ingestion_projection(*, detail: dict[str, Any]) -> WorkflowProjection:
    ...
    if converted:
        ...
        operator_message = (
            "Open the converted draft when you need to continue structured authoring or review."
        )
        tone = "approved"
        ...
        operator_message = (
            "Review mapping conflicts and missing sections before deciding whether to convert."
        )
        tone = "warning"
    elif mapped:
