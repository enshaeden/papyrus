...
class DocumentationContractTests(unittest.TestCase):
    def test_reference_docs_describe_ui_boundary_and_semantic_hooks(self) -> None:
        ...
        self.assertIn("/read", operator_web_ui)
        self.assertIn("/write/new", operator_web_ui)
        self.assertIn("/review", operator_web_ui)
        self.assertIn("/admin/overview", operator_web_ui)
        self.assertIn("400 bad request", operator_web_ui)
        self.assertIn("post /write/object/{object_id}/start", operator_web_ui)
        self.assertIn("no separate page-header actor banner contract", operator_web_ui)
        self.assertIn("request-scoped role context", operator_web_ui)
    ...
    def test_getting_started_documents_explicit_source_workspace_boundary(self) -> None:
        ...
        self.assertIn("/review", getting_started)
        self.assertIn("/read", getting_started)
        self.assertIn("/admin/overview", getting_started)
    ...
    def test_canonical_decision_docs_lock_experience_architecture(self) -> None:
        ...
        layout_contracts = (
            (ROOT / "decisions" / "layout-contracts.md").read_text(encoding="utf-8").lower()
        )
        component_contracts = (
        ...
        self.assertIn("canonical request identity", experience_architecture)
        self.assertIn("reader -> `/read`", experience_architecture)
        self.assertIn("`local.operator` maps to operator", experience_architecture)
        self.assertIn("`local.reader` maps to reader", experience_architecture)
        self.assertIn("top bar", layout_contracts)
        self.assertIn("contextual only", layout_contracts)
        self.assertIn("request-scoped role context", layout_contracts)
    ...
    def test_lifecycle_decision_uses_explicit_state_machines(self) -> None:
        ...
        self.assertIn("`published` is not a canonical revision review state", lifecycle)
