...
Runtime landing defaults:

- Reader lands on `/read`
- Operator lands on `/review`
- Admin lands on `/admin/overview`
...
- Shared web routes use canonical paths such as `/read`, `/write`, `/import`, `/review`, `/governance`, `/admin/overview`, and `/admin/audit`.
- Request-scoped role context comes from runtime or authenticated identity, not from URL prefixes.
...
`scripts/serve_api.py` remains an operator-oriented local API surface. The JSON API remains an operator-oriented local surface. It is not part of the role-scoped web experience contract, and any future role-scoped API contract requires a separate decision and migration.
...
- Review and governance playbook: [knowledge/manage.md](knowledge/manage.md)
